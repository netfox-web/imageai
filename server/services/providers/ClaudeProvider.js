...
export class ClaudeProvider extends AIProviderInterface {
  ...
  async generatePost(task) {
    return this.withFallback('generatePost', [task], async () => {
      const result = await this.generateText({ prompt: buildPostPrompt(task), model: task.requested_model || config.claudeModel });
      if (!result.ok) {
        const error = new Error(result.error_message || 'Claude post generation failed.');
