...
function SharePage({ token }) {
  ...
  const textAsset = asset.asset_kind === 'text' || String(asset.mime_type || '').startsWith('text/');
  if (textAsset) {
    const downloadUrl = asset.download_url || asset.content_url || '';
    return (
      <main className="mx-auto max-w-4xl px-4 py-8">
        <PageTitle title={asset.product_name || 'Shared asset'} subtitle="Public copywriting share" />
        <div className="panel">
          <pre className="max-h-[70vh] overflow-auto whitespace-pre-wrap break-words rounded-lg border border-neutral-200 bg-white p-4 text-sm leading-7 text-neutral-800">
            {asset.text || 'No copy output available.'}
          </pre>
          <div className="mt-4 flex flex-wrap gap-2">
            <a href={downloadUrl} download={asset.download_filename || true} className="btn btn-yellow gap-2"><Download className="h-4 w-4" />Download TXT</a>
            <button className="btn btn-ghost" onClick={() => { window.location.href = `/feedback?asset_url=${encodeURIComponent(downloadUrl)}`; }}>Report copy issue</button>
          </div>
          <div className="mt-3 text-sm font-bold text-neutral-500">Copywriting output generated by imageai.tw</div>
